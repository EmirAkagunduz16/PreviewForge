...
- M6-QUERY-API, M6-ENV-VARS, M6-LOG-DURABILITY, M6-SSE-API, and M6-DASHBOARD are complete with real PostgreSQL/HTTP/browser evidence; M6-ACCEPTANCE is next. Full M5 health-classification drift remains a prerequisite to integrated M6 acceptance. See the [M6 Dashboard report](../reports/session-2026-09-16-m6-dashboard.md) and [active backlog](../backlog/active.md).
...
Use [the roadmap](../delivery/roadmap.md) for milestone status and [the active backlog](../backlog/active.md) for the next unfinished action. M3 through M5 are complete; M6 dashboard/live-log work is current, with M6-ACCEPTANCE next and M5 health acceptance drift gating the integrated M6 run. Read this file when a durable invariant or decision is needed; it is an index, not a progress log. Update it only for durable decisions, verified milestones, or security-relevant lessons, then mirror the durable summary to the linked VictusOS note.
