...
- M4 is active. Source acquisition, the rootless BuildKit adapter, desired-SHA-guarded digest persistence, hosted-runner provisioning, checksum-pinned binaries, and the real build/push/digest harness are implemented. Hosted run `34854326618` proved the custom confined AppArmor path fails naturally at `getsubids`/`newuidmap` execution, so it is retired from the canonical design. The runner now requires Ubuntu 24.04's packaged `/usr/bin/rootlesskit flags=(unconfined) { userns, }` per-binary profile without editing or reloading it. Hosted validation of this new path is pending.
...
Use [the roadmap](../delivery/roadmap.md) for milestone status and [the active backlog](../backlog/active.md) for the next unfinished action. M3 is complete; M4 rootless image build is active and blocked pending real hosted validation of the packaged RootlessKit AppArmor path recorded in the [M4 host-prerequisite incident](../reports/incident-2026-09-14-m4-rootless-host-prerequisite.md). Read this file when a durable invariant or decision is needed; it is an index, not a progress log. Update it only for durable decisions, verified milestones, or security-relevant lessons, then mirror the durable summary to the linked VictusOS note.
