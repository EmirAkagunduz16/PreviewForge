...
- M4 is active. Source acquisition, the rootless BuildKit adapter, desired-SHA-guarded digest persistence, hosted-runner provisioning, checksum-pinned binaries, and the real build/push/digest harness are implemented. Hosted run `34860645607` proved the packaged Ubuntu 24.04 RootlessKit path, socket authorization, normal-user BuildKit workers, fixture build/push, immutable digest verification, and cleanup; hosted run `34864066506` additionally proved private source fetch, real BuildKit/registry persistence, and stale-SHA supersession. Public source, failure, timeout, duplicate-delivery, and credential-boundary cases remain open.
...
Use [the roadmap](../delivery/roadmap.md) for milestone status and [the active backlog](../backlog/active.md) for the next unfinished action. M3 is complete; M4 rootless image build is active with hosted infrastructure and private source/build/registry validation proven by runs `34860645607` and `34864066506`, while the full acceptance matrix remains open. Read this file when a durable invariant or decision is needed; it is an index, not a progress log. Update it only for durable decisions, verified milestones, or security-relevant lessons, then mirror the durable summary to the linked VictusOS note.
