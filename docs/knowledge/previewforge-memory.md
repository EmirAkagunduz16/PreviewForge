...
- M4 is active. Source acquisition, the rootless BuildKit adapter, desired-SHA-guarded digest persistence, hosted-runner provisioning, checksum-pinned binaries, and the real build/push/digest harness are implemented. The hosted acceptance is deliberately iterative and fail-closed: baseline, provisioning, profile loading, config staging, explicit confined-profile selection, and private RootlessKit state selection are proven. The repository profile now permits only the three reads denied during UID/GID-map setup in run `34850233883`; local parser, shell, and full repository checks pass, but the updated hosted run is pending. M4 is not complete until the hosted build → push → immutable digest oracle passes.
