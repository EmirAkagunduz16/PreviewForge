...
- M8 local hardening implementation is integrated. Deterministic fixtures and the restore/outbox drill, direct failure-injected API/worker tests, metrics/traces, Prometheus/Grafana/Tempo evidence, and real kind/Envoy acceptance are verified; `pnpm check` passes. The local acceptance gate remains active only because this host has no `buildkitd`, `buildctl`, or BuildKit socket for the required rootless BuildKit scenario. AWS EKS/ECR is explicitly deferred as blocked M9 work until an approved cost boundary exists. See the [M8 execution plan](../plans/m8-hardening-cloud-demo.md), [canonical M8 report](../reports/session-2026-09-17-m8-acceptance.md), and [active backlog](../backlog/active.md).
...
Use [the roadmap](../delivery/roadmap.md) for milestone status and [the active backlog](../backlog/active.md) for the next unfinished action. M3 through M7 are complete; M8 local hardening is active with the BuildKit prerequisite open, and the M9 cloud demo is deferred. Read this file when a durable invariant or decision is needed; it is an index, not a progress log. Update it only for durable decisions, verified milestones, or security-relevant lessons, then mirror the durable summary to the linked VictusOS note.
