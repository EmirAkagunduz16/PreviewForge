...
Use [the roadmap](../delivery/roadmap.md) for milestone status and [the active backlog](../backlog/active.md) for the next unfinished action. M3 is complete; M4 rootless image build is active with hosted infrastructure validation proven by run `34860645607`, while the full source/build/registry acceptance matrix remains open. Read this file when a durable invariant or decision is needed; it is an index, not a progress log. Update it only for durable decisions, verified milestones, or security-relevant lessons, then mirror the durable summary to the linked VictusOS note.
